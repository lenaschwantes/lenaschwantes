# Helena Schwantes

**Data Engineer & AI Integration Specialist**  
Building production-grade data pipelines and LLM-powered applications

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=flat&logo=linkedin)](your-linkedin-url)
[![Email](https://img.shields.io/badge/Email-helena%40schwantes.com.br-EA4335?style=flat&logo=gmail)](mailto:helena@schwantes.com.br)
[![Portfolio](https://img.shields.io/badge/Portfolio-View-4285F4?style=flat&logo=google-chrome)](your-portfolio-url)

---

## 🎯 About Me

Data Engineer specializing in scalable data infrastructure and LLM integration. I build high-performance pipelines, cloud architectures, and agentic AI workflows that solve real engineering problems at scale.

**Track record:** Optimized enterprise systems processing 100M+ records daily, reducing critical query times from 45min → 3min (94% improvement)

**Currently exploring:** Multi-agent LLM systems, RAG architectures, and production-ready FastAPI applications  
**Location:** Florianópolis, Brazil 🇧🇷 | Open to remote opportunities worldwide

## 🛠️ Technical Skills

### Languages & Frameworks
![Python](https://img.shields.io/badge/Python-Expert-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-Expert-4479A1?style=flat&logo=postgresql&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-Advanced-009688?style=flat&logo=fastapi&logoColor=white)

### Data Engineering
**Cloud:** AWS (S3, Glue, Lambda, Redshift) • Azure • GCP  
**Databases:** Snowflake • PostgreSQL • MongoDB • MySQL  
**Big Data:** Apache Spark • Kafka • Kafka Connect  
**Tools:** Docker • Airflow • dbt

### AI & LLMs
**Frameworks:** LangChain • CrewAI • OpenAI API  
**Techniques:** RAG • Prompt Engineering • Multi-agent systems • Vector databases (FAISS, Pinecone)

---

## 📊 GitHub Stats

![Helena's GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=default)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact)

---

## 🌍 Languages

🇧🇷 **Portuguese** — Native  
🇺🇸 **English** — Fluent

---

## 📫 Let's Connect

Open to **Senior Python/LLM Engineer** positions and collaborative projects in data engineering and AI integration.

- 💼 [LinkedIn](your-linkedin-url)
- 📧 helena@schwantes.com.br

---

<div align="center">

### 💡 Open to collaborating on data engineering and LLM integration projects

*"Feito > Perfeito"* — Execution over perfection

</div>
